...
def run_leiden_all(distance_matrix,meta_tSNE,**kwargs):
    ...
    meta_tSNE,upairs = leiden_clustering(meta_tSNE,respar, pairs,distance_matrix)
...
def leiden_clustering(meta_tSNE,resolution_parameter, pairs, distance_matrix):
    start = time.time()
    print('----------------------------------------------------')
...
def plot_leiden_clusters(meta_tSNE,unique_pairs,revdict,**kwargs):
    ...
    meta_tSNE.plot.scatter('tSNE_1','tSNE_2',figsize=(10,10),ax=ax,color=colors,s=meta_tSNE.nGene.divide(80),alpha=.3)#,label=colors.keys)
    ...
    plt.show()
    f.savefig(savedir+'/tSNE_LeidenDegree_resPar='+str(respar)+'NN='+str(NN)+'.png')
    ...
    plt.gcf().subplots_adjust(right=0.88)
    plt.show()
    f.savefig(savedir+'/tSNE_LeidenDegree_allEdges_resPar='+str(respar)+'NN='+str(NN)+'.png')
